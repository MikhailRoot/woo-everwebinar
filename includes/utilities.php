 ...
 * @return array
 */
function get_webinar_registration_results_from_order( $post = null ) {
	...
	if ( ( $post instanceof \WP_Post && ( 'shop_order' === $post->post_type ) ) || $post instanceof \WC_Abstract_Order ) {
		...
		$result = json_decode( get_post_meta( $post->ID, 'everwebinar_registration_result', true ) );
		...
		if ( is_object( $result ) ) {
			...
			$result = array( $result );
			update_post_meta( $post->ID, 'everwebinar_registration_result', wp_json_encode( $result ) );
		}
...
/**
 * Check if $post as \WP_Post or post_id or global $post is order and have everwebinar products in it.
 *
 * @param null|int|\WP_Post|\WC_Order $post Post of Product.
 * @return bool
 */
function order_has_webinars( $post = null ) {
	...
	if ( $order instanceof \WC_Abstract_Order ) {
		...
		foreach ( $products as &$product_item ) {
			...
			if ( 'everwebinar' === $product->product_type ) {
				return true;
			}
		}
	}
 ...
 * @return array
 */
function order_get_webinars( $post = null ) {
	$order = null;
...
/**
 * Returns Last WC_Order->ID from current users, order history where he ordered everwebinar webinar.
 *
 * @return int|null
 */
function get_current_user_last_order_id_with_webinar() {
	...
	if ( $customer instanceof \WP_User && $customer->ID > 0 ) {
		...
		foreach ( $customer_orders as $customer_order ) {
			if ( order_has_webinars( $customer_order ) ) {
...
/**
 * Returns array of WP_Post or empty array of orders which has everwebinar webinars as product items.
 *
 * @return array
 */
function get_current_user_orders_with_webinars() {
 ...
 * @return string
 */
function make_placeholder_replacements_for_webinar( $content = '', $webinar_reg_result = array() ) {
