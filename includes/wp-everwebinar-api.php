 ...
 */
function list_webinars( $api_key ) {

	if ( empty( $api_key ) ) {
		return new \WP_Error( 'empty_api_key', 'Set API key' );
	}

	$response = wp_remote_post(
		'https://api.webinarjam.com/everwebinar/webinars',
		array(
			'method' => 'POST',
 ...
 * @return array|\WP_Error
 */
function get_webinar_data( $api_key, $webinar_id ) {
 ...
 * @param int    $schedule          Schedule param for webinar (wasn't used yet by everwebinar docs).
 * @return array|\WP_Error
 */
function register_user_to_webinar( $api_key, $webinar_id, $user_email, $user_first_name, $user_last_name, $schedule = 0 ) {
